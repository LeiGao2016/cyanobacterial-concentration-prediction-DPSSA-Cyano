...
def evaluation_metric(y_test,y_hat):
    ...
    MAPE = metrics.mean_absolute_percentage_error(y_test,y_hat)
...
def data_split(sequence, n_timestamp):
    ...
    for i in range(len(sequence)):
        ...
        seq_x = sequence[i:end_ix+1, 0:sequence.shape[1]]
        seq_y = sequence[end_ix, sequence.shape[1]]
        X.append(seq_x)
        y.append(seq_y)
...
def prepare_data(series, n_test, n_in, n_out):
    ...
    train, test = supervised_data.loc[:1200, :], supervised_data.loc[1200:, :]
    return train, test
